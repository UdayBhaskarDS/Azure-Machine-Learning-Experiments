...
def train_model(data):  
    from sklearn.linear_model import LogisticRegression     # Here we are fitting training data to the model.
    model = LogisticRegression()                            # Algorithm is LogisticRegression
    ...
    decisions = (model.predict_proba(x_test) >= mythreshold).astype(int)
...
for i in C_param_range:
    ...
    performence_table.iloc[j,1] = roc_auc_score
    
    if lr is not None: 
       
        run.log_list(name="roc_auc_score",
                     value=lr.get("roc_auc_score"),
                     description="model roc_auc_score logged.")
